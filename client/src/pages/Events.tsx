    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-5xl mx-auto">
      <div className="flex items-start justify-between mb-6 sm:mb-8 gap-4">
        <div>
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-bold text-zinc-900">Events</h1>
        </div>
      ...
      {/* Tabs */}
      <div className="flex gap-4 sm:gap-6 border-b border-[#e2e0db] mb-6 overflow-x-auto scrollbar-none">
        {STATUS_TABS.map((tab) => (
          <button
            key={tab.value}
            onClick={() => setActiveTab(tab.value)}
            className={`pb-3 text-[11px] font-semibold uppercase tracking-widest transition-colors border-b-2 -mb-px whitespace-nowrap ${
              activeTab === tab.value
